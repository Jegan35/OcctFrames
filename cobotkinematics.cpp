...
// =====================================================================
// PURE ANALYTICAL KINEMATICS (UNIVERSAL ROBOTS)
// =====================================================================
namespace ur {
...
static Model P = {"CUSTOM", 0.089159, -0.42500, -0.39225, 0.10915, 0.09465, 0.0823};
...
int inverse(const double T[16], double* q_sols, double q6_des)
{
    const double d1 = P.d1, a2 = P.a2, a3 = P.a3;
    const double d4 = P.d4, d5 = P.d5, d6 = P.d6;
    int num_sols = 0;
...
// =====================================================================
// COBOT KINEMATIC CLASS METHODS
// =====================================================================

int CobotKinematic::RebuildChain(double bx, double bz, double az, double ez, double fx, double wx, double fy)
{
    ur::setCustomModel(bz/1000.0, -az/1000.0, -ez/1000.0, fx/1000.0, wx/1000.0, fy/1000.0);

    KDLChain = KDL::Chain();
    KDLChain.addSegment(KDL::Segment("J1", KDL::Joint(KDL::Joint::RotZ), KDL::Frame::DH(0.0, M_PI_2, bz, 0.0)));
    KDLChain.addSegment(KDL::Segment("J2", KDL::Joint(KDL::Joint::RotZ), KDL::Frame::DH(az, 0.0, 0.0, 0.0)));
    KDLChain.addSegment(KDL::Segment("J3", KDL::Joint(KDL::Joint::RotZ), KDL::Frame::DH(ez, 0.0, 0.0, 0.0)));
    KDLChain.addSegment(KDL::Segment("J4", KDL::Joint(KDL::Joint::RotZ), KDL::Frame::DH(0.0, M_PI_2, fx, 0.0)));
    KDLChain.addSegment(KDL::Segment("J5", KDL::Joint(KDL::Joint::RotZ), KDL::Frame::DH(0.0, -M_PI_2, wx, 0.0)));
    KDLChain.addSegment(KDL::Segment("J6", KDL::Joint(KDL::Joint::RotZ), KDL::Frame::DH(0.0, 0.0, fy, 0.0)));

    KDLJointMin.resize(6); KDLJointMax.resize(6); KDLJointCur.resize(6); KDLJointInit.resize(6);

    KDLJointMin(0) = -180.0 * toRadians; KDLJointMax(0) =  180.0 * toRadians;
    KDLJointMin(1) = -90.0  * toRadians; KDLJointMax(1) =  90.0  * toRadians;
    KDLJointMin(2) = -160.0 * toRadians; KDLJointMax(2) =  160.0 * toRadians;
    KDLJointMin(3) = -100.0 * toRadians; KDLJointMax(3) =  100.0 * toRadians;
    KDLJointMin(4) = -180.0 * toRadians; KDLJointMax(4) =  180.0 * toRadians;
    KDLJointMin(5) = -360.0 * toRadians; KDLJointMax(5) =  360.0 * toRadians;
